namespace HREngine.Bots
{
    ...
    public class PenalityManager
    {
            ...
            if (target.own && !target.isHero)
            {
                if (DamageTargetDatabase.ContainsKey(name) || (p.anzOwnAuchenaiSoulpriest >= 1 && HealTargetDatabase.ContainsKey(name)))
                {
                    ...
                    if (m.Hp > dmg)
                    {
                        switch (m.name)
                        {
                            case CardDB.cardName.gurubashiberserker: return 0; 
                            case CardDB.cardName.axeflinger: return 0; 
                            case CardDB.cardName.gahzrilla: return 0; 
                    ...
                    else
                    {
                        if (p.isLethalCheck && dmg == 1 && p.enemyHero.Hp < 3)
                        {
                            switch (m.name)
                            {
                                case CardDB.cardName.lepergnome: return 0; 
                                case CardDB.cardName.axeflinger: return 0; 
                            }
                        }
                ...
                if (DamageTargetSpecialDatabase.ContainsKey(name))
                {
                    ...
                    if (m.Hp > dmg)
                    {
                        ...
                        foreach (Handmanager.Handcard hc in p.owncards) // no pen if we have battlerage for example
                        {
                            switch (hc.card.name)
                            {
                                case CardDB.cardName.battlerage: return pen; 
                                case CardDB.cardName.rampage: return pen; 
                                case CardDB.cardName.bloodwarriors: return pen; 
                            }
                        }
                    }
                    pen = 500;
        ...
        private int getRandomPenaltiy(CardDB.Card card, Playfield p, Minion target)
        {
            ...
            int cards = this.randomEffects.ContainsKey(card.name) ? this.randomEffects[card.name] : this.cardDrawBattleCryDatabase[card.name];
        ...
        private int getSpecialCardComboPenalitys(CardDB.Card card, Minion target, Playfield p)
        {
            ...
            {   //----------------------通用---------------------
                ...
                //火舌
                case CardDB.cardName.flametonguetotem:
                    if (p.ownMinions.Count == 0) return 100;break;
                //先祖之魂
            ...
            //-------------------------------------------奥秘法----------------------------------------------                
            // 手上/场上是否有奥秘
            bool hasSecret = (p.ownSecretsIDList.Count < 1);
            foreach (Handmanager.Handcard hc in p.owncards)
            {
            ...
            foreach(Handmanager.Handcard hc in p.owncards)
            {
                ...
                if (m.name == CardDB.cardName.kabalcrystalrunner || m.name == CardDB.cardName.cloudprince) found1 = true;
            ...
            switch (card.name)
            {
                ...
                case CardDB.cardName.kaballackey:
                    ...
                    {
                        // 有奥秘在2费以上时打出
                        if (p.mana >= 1 && hc.card.Secret && hc.manacost >= 2 ) return -60;
                    }
                    return 20;
				//肯瑞托法师
				case CardDB.cardName.kirintormage:
					foreach (Handmanager.Handcard hc in p.owncards)
                    {
                        // 有奥秘在2费以上时打出
                        if (p.mana >= 1 && hc.card.Secret && hc.manacost >= 2 ) return -60;
                ...
                case CardDB.cardName.arcanologist:
					...
					break;
                //厄运信天翁
                case CardDB.cardName.badluckalbatross:
                ...
                case CardDB.cardName.ancientmysteries:
                    if (p.ownSecretsIDList.Count < 1 && p.ownMaxMana >= 2)
                    {
                        foreach(Handmanager.Handcard hc in p.owncards)
                        {
                            CardDB.Card m = hc.card;
                            if(m.name==CardDB.cardName.kabalcrystalrunner)
                            {
                                return -15;
                            }
                            else if(m.name==CardDB.cardName.cloudprince && p.ownMaxMana >= 7)
                            {
                                return -20;
                            }
                            else if(m.name==CardDB.cardName.medivhsvalet && p.ownMaxMana >= 4)
                            {
                                return -20;
                            }
                        }
                ...
                break;
                //爆炸符文
                case CardDB.cardName.explosiverune:
                    bool b1=false;
                    if(p.ownSecretsIDList.Count<1)
                    {
                        ...
                        {
                            CardDB.Card m = hc.card;
                            if(m.name==CardDB.cardName.kabalcrystalrunner||m.name==CardDB.cardName.cloudprince||m.name==CardDB.cardName.medivhsvalet) b1=true;
                        }
                    }
                    if(b1) return -10;
                    if(p.enemyHero.Hp<10) return -6;
                    return -5;
                ...
                //虚空之风传送门
                case CardDB.cardName.netherwindportal:
                    if (found1) return -10;
                    else return -5;
                    break;
                
                //绿洲盟军
                case CardDB.cardName.oasisally:
                    if (found1) return -10;
                    else return -5;
                    break;
                 //寒冰屏障
                    ...
                    if(p.ownSecretsIDList.Count<1)
                            {
								...
								{
									CardDB.Card m = hc.card;
                                    if (m.name == CardDB.cardName.kabalcrystalrunner || m.name == CardDB.cardName.cloudprince||m.name==CardDB.cardName.medivhsvalet) b2 = true;
                                }
                            }
                    if (b2) return -8;
                    if (p.ownHero.Hp < 15) return -11;
                    else return -1;
                    break;
                //法术反制
                ...
                case CardDB.cardName.riggedfairegame:
                    if (p.owncards.Count <= 4) return -11;
                    if (p.owncards.Count >= 8) return 20;
                    return -10;
                    break;
                //暗金教水晶侍女
                case CardDB.cardName.kabalcrystalrunner:
                    if (card.cost >=6 ) return 10;
                    else if (card.cost >=4 ) return 0;
                    else if (card.cost >=2 ) return -10;
                    else return -90;
                    break;
                //隐秘咒术师
                case CardDB.cardName.occultconjurer:
                    // 当白板打
                    if (p.ownSecretsIDList.Count < 1) return -2;
                    else
                        return -12;
                    break;
                //暗月先知赛格
                case CardDB.cardName.saygeseerofdarkmoon:
                    int ownCards = p.owncards.Count;
                    // TODO 这里需要先定义已经使用过的奥秘数量
                    int drawCards = p.secretsTriggeredSinceRecalc;
                    // int drawCards = 3;
                    if (ownCards >= 4) return 20;
                    if (drawCards + ownCards <= 11) return -15;
                    else return (drawCards + ownCards - 10);
                    break;

                //云雾王子
                case CardDB.cardName.cloudprince:
                    if (p.ownSecretsIDList.Count < 1) return 80;
					if(target.isHero && p.enemyHero.Hp < 10) return -25;
					if(target.isHero && p.enemyHero.Hp < 18) return -20;
                    if (target.isHero) return -11;
					return -10;
                    break;
                //艾露尼斯
                ...
                case CardDB.cardName.fireblast:
                    if (p.enemyHero.Hp < 10 && target.isHero) return -1;
                    return 0;
                    break;
				// 火球术
				case CardDB.cardName.fireball:
					if(target.isHero && p.enemyHero.Hp < 10) return -19;
                    if (target.isHero) return -4;
                    break;
            } 
            ...
            switch (card.name)
            {
                ...
                case CardDB.cardName.divinefavor:
                    // 剩余卡牌不足，停止抽卡
                    if (prozis.ownDeckSize < 5) return 100;
                    // 抽牌越多优先度越高
                ...
                case CardDB.cardName.calltoarms:
                    // 判断牌库中2费以下随从数量
                    int count = 0;
                    foreach(KeyValuePair<CardDB.cardIDEnum, int>kvp in prozis.turnDeck )
                    {
            ...
            switch (card.name)
            {
                ...
                //神恩术 divinefavor
                case CardDB.cardName.divinefavor:
                    if (p.tempanzEnemyCards - p.owncards.Count >= 2) return -50;
                    else return 50;
                    break;
                ...
                //亮石技师
                //原则同上
                case CardDB.cardName.glowstonetechnician:
                    {
                        int iii = 0;
                        foreach (Handmanager.Handcard hc in p.owncards)
                        {
                    ...
                    break;
                //通电机器人
                case CardDB.cardName.galvanizer:
                    {
                        int iii = 0;
                        foreach (Handmanager.Handcard hc in p.owncards)
                        {
                ...
                * */
                ...
                case CardDB.cardName.vessina:
                    if((p.ueberladung > 0 || p.lockedMana > 0) && p.ownMinions.Count > 0) return 2-3*p.ownMinions.Count;
                    break;
                //分裂战斧
                ...
                //沙暴
                case CardDB.cardName.sandstormelemental:
                    if (p.enemyMinions.Count == 1) return -3;
                    if (p.enemyMinions.Count >= 2) return -10;
                    else return 0;

                //甲虫
                case CardDB.cardName.beetlestampede:
                    return -5;
                ...
                //国王的赎金
                case CardDB.cardName.kingsransom:
                    return -50;

                //女巫跟班
                case CardDB.cardName.witchylackey:
                    if (p.ownMinions.Count >= 1) return -5;
                    return 10;

                //无面跟班
                case CardDB.cardName.facelesslackey:
                    if (p.ownMinions.Count <= 5) return -5;
                    return 10;

                //龙族跟班
                case CardDB.cardName.draconiclackey:
                    if (p.ownMinions.Count >= 1) return -5;
                    return 10;

                //地精跟班
                case CardDB.cardName.goblinlackey:
                    if (p.ownMinions.Count >= 1) return -5;
                    return 10;

                //狗头人跟班
                case CardDB.cardName.koboldlackey:
                    return -5;

                //泰坦造物跟班
                case CardDB.cardName.titaniclackey:
                    if (p.ownMinions.Count >= 1) return -5;
                    return 10;
                //活动喷泉
                case CardDB.cardName.walkingfountain:
                    if (p.enemyMinions.Count >= 2) return -10;
                    else return -5;
                ...
                //蟾蜍雨
                case CardDB.cardName.rainoftoads:
                ...
                case CardDB.cardName.eviltotem:
                    return -5;
				case CardDB.cardName.totemicsummons:
                ...
                //终极封印
                case CardDB.cardName.thefinalseal:
                    return -10;
                //第一封印
                case CardDB.cardName.thefirstseal:
                    return -10;
                //第二封印
                case CardDB.cardName.thesecondseal:
                    return -10;
                //第三封印
                case CardDB.cardName.thethirdseal:
                    return -10;
                //第四封印
                case CardDB.cardName.thefourthseal:
                    return -10;
                //吞噬者阿扎里
                case CardDB.cardName.azarithedevourer:
                    return -10;
                //深渊魔物
                case CardDB.cardName.thingfrombelow:
                    if (p.ownAbilityReady && p.ownMinions.Count <= 5) return 500;
                    break;
            ...
            if ((p.ownHeroAblility.card.name == CardDB.cardName.totemiccall || p.ownHeroAblility.card.name == CardDB.cardName.totemicslam) && p.ownAbilityReady == false)
            {
                foreach (Action a in p.playactions)
                {
                    ...
                    if (a.actionType == actionEnum.playcard && a.card.card.name == CardDB.cardName.splittingaxe) return -1;
                }
                    ...
                    else
                    {
                        ...
                        if (target.handcard.card.battlecry)
                        {
                            ...
                            switch (target.name)
                            {
                                ...
                                case CardDB.cardName.kingselekk: return 0; 
        ...
        private int getPlayCardSecretPenality(CardDB.Card c, Playfield p)
        {
            ...
            switch (c.name)
            {
                case CardDB.cardName.flare: return 0;
                case CardDB.cardName.eaterofsecrets: return 0;
                case CardDB.cardName.kezanmystic: 
...
//                    }
        ...
        private void setupPriorityList()
        {
            ...
            // 螃蟹骑士
            priorityDatabase.Add(CardDB.cardName.crabrider, 4);
            // 哨所
            priorityDatabase.Add(CardDB.cardName.farwatchpost, 5);
            priorityDatabase.Add(CardDB.cardName.crossroadswatchpost, 5);
            priorityDatabase.Add(CardDB.cardName.morshanwatchpost, 5);
            // 鱼人
            priorityDatabase.Add(CardDB.cardName.firemancerflurgl, 6);
            priorityDatabase.Add(CardDB.cardName.tinyfinscaravan, 6);
            priorityDatabase.Add(CardDB.cardName.lushwaterscout, 6);
        }
        ...
        private void setupUsefulNeedKeepDatabase()
        {
            ...
            UsefulNeedKeepDatabase.Add(CardDB.cardName.stargazerluna, 20);
            // 螃蟹骑士
            UsefulNeedKeepDatabase.Add(CardDB.cardName.crabrider, 100);
            // 哨所
            UsefulNeedKeepDatabase.Add(CardDB.cardName.farwatchpost, 10);
            UsefulNeedKeepDatabase.Add(CardDB.cardName.crossroadswatchpost, 10);
            UsefulNeedKeepDatabase.Add(CardDB.cardName.morshanwatchpost, 10);
            // 鱼人
            UsefulNeedKeepDatabase.Add(CardDB.cardName.firemancerflurgl, 20);
            UsefulNeedKeepDatabase.Add(CardDB.cardName.tinyfinscaravan, 20);
            UsefulNeedKeepDatabase.Add(CardDB.cardName.lushwaterscout, 20);
        }
        ...
        private void setupSpecialMins()
        {
            ...
            // 螃蟹骑士
            specialMinions.Add(CardDB.cardName.crabrider, 0);
            // 哨所
            specialMinions.Add(CardDB.cardName.farwatchpost, 0);
            specialMinions.Add(CardDB.cardName.crossroadswatchpost, 0);
            specialMinions.Add(CardDB.cardName.morshanwatchpost, 0);
            // 鱼人
            specialMinions.Add(CardDB.cardName.firemancerflurgl, 0);
            specialMinions.Add(CardDB.cardName.tinyfinscaravan, 0);
            specialMinions.Add(CardDB.cardName.lushwaterscout, 0);
        }
        ...
        private void setupEnemyTargetPriority()
        {
            ...
            // 螃蟹骑士
            priorityTargets.Add(CardDB.cardName.crabrider, 10);
            // 哨所
            priorityTargets.Add(CardDB.cardName.farwatchpost, 10);
            priorityTargets.Add(CardDB.cardName.crossroadswatchpost, 10);
            priorityTargets.Add(CardDB.cardName.morshanwatchpost, 10);
            // 鱼人
            priorityTargets.Add(CardDB.cardName.firemancerflurgl, 10);
            priorityTargets.Add(CardDB.cardName.tinyfinscaravan, 10);
            priorityTargets.Add(CardDB.cardName.lushwaterscout, 10);
        }
